...
const VERDICT_MATCHED_PARTIAL =
  'Verdict: everything we could check matched the simulation. Rows marked "not checked" ' +
  'are not recorded in a transaction receipt, so they could not be independently confirmed.';
    ...
    blocks.push(
      [
        ...
        ...input.postflight.lines.map((l) => {
          const mark =
            l.status === 'matched' ? '✓' : l.status === 'mismatched' ? '✗' : '– not checked';
